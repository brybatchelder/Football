...
describe("navigation route matching", () => {
  it("matches exact routes and their nested detail pages", () => {
    ...
    expect(routeMatches("/league/rosters/history", "/league/rosters")).toBe(true);
    expect(routeMatches("/league/power-rankings", "/league/rosters")).toBe(false);
  });

  it("selects only the most specific item when parent routes also match", () => {
    expect(mostSpecificNavigationItem("/league/lifecycle", items)?.[0]).toBe("Lifecycle");
    expect(mostSpecificNavigationItem("/league/rosters/history", items)?.[0]).toBe("Rosters");
