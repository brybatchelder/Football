...
export const powerRankingModelV1: PowerRankingWeights = {
  lineup: 0.7,
  depth: 0.3,
};
...
function normalize(inputs: PowerRankingInput[], key: keyof PowerRankingInput) {
  ...
  return new Map(
    inputs.map((input) => [
      input.id,
      max === min ? 50 : ((Number(input[key]) - min) / (max - min)) * 100,
    ]),
  );
}

export function powerRankings(
  inputs: PowerRankingInput[],
  weights = powerRankingModelV1,
) {
  ...
  return inputs
    .map((input) => {
      const score =
        ((lineup.get(input.id) ?? 0) * weights.lineup +
          (depth.get(input.id) ?? 0) * weights.depth) /
        totalWeight;
      return {
        ...input,
        score: Number(score.toFixed(1)),
        components: {
          lineup: Number((lineup.get(input.id) ?? 0).toFixed(1)),
          depth: Number((depth.get(input.id) ?? 0).toFixed(1)),
          offense: Number((offense.get(input.id) ?? 0).toFixed(1)),
          defense: Number((defense.get(input.id) ?? 0).toFixed(1)),
        },
      };
    })
    .sort(
