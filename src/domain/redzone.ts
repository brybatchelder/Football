...
export function inferEvents(
  previous: NormalizedPlayerSnapshot,
  current: NormalizedPlayerSnapshot,
  rules: ScoringRule[],
): InferredEvent[] {
  if (
    ...
    throw new Error("Snapshots must describe the same player and game");
  ...
  if (hasCorrection)
    return [
      event(
        ...
        scored.points,
        "correction",
        "Stat correction from upstream provider",
      ),
    ];
  const touchdown = (
    key: keyof NormalizedStats,
    type: InferredEvent["eventType"],
  ) => {
...
function event(
  ...
  eventType: InferredEvent["eventType"],
  stats: NormalizedStats,
  fantasyPointDelta: number,
  confidence: EventConfidence,
  summary: string,
...
function labelFor(type: InferredEvent["eventType"], delta: NormalizedStats) {
  const yards =
    type === "receiving_touchdown"
      ? delta.receiving_yards
      : type === "rushing_touchdown"
        ? delta.rushing_yards
        : delta.passing_yards;
