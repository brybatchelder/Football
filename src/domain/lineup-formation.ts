...
export function lineupFormation(starters: FormationStarter[]) {
  const offenseFlexes = starters
    .filter((starter) => starter.slot === "OFLEX")
    .map((starter) => starter.position);
  const defenseFlexes = starters
    .filter((starter) => starter.slot === "DFLEX")
    .map((starter) => starter.position);
  return {
    offense: formation(
      offenseFlexes,
...
function formation(
  ...
  const counts = Object.fromEntries(
    displayOrder.map((position) => [
      position,
      starters.filter((starter) => starter.position === position).length,
    ]),
  ) as Record<Position, number>;
  const personnel = displayOrder
    .map((position) => `${counts[position]}${position}`)
    .join(" · ");
  const key = [...flexes].sort().join(",");
  const validFlexes =
    flexes.length === 2 &&
    flexes.every((position) => eligible.includes(position));
  return {
    name: validFlexes ? names[key] : "Lineup in progress",
    personnel,
    incomplete: !validFlexes,
  };
}
...
function favorite(starters: FormationStarter[], positions: Position[]) {
  const counts = positions.map((position) => ({
    position,
    count: starters.filter((starter) => starter.position === position).length,
  }));
  const high = Math.max(...counts.map((item) => item.count));
  return counts
    .filter((item) => item.count === high && high > 0)
