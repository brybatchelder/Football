...
function splitName(displayName: string) {
  ...
  return {
    firstName: pieces[0] ?? "Unknown",
    lastName: pieces.slice(1).join(" ") || "Unknown",
  };
}

export function normalizeNflverseData(
  playerRows: CsvRow[],
  rosterRows: CsvRow[],
): NflversePlayer[] {
  ...
  return playerRows.map((row) => {
    ...
    const displayName =
      cell(row, "display_name", "full_name", "football_name", "short_name") ||
      cell(roster, "full_name", "player_name");
    const fallback = splitName(displayName);
    const nflStatus =
      cell(roster, "status", "roster_status") || cell(row, "status");
    return {
      gsisId,
      displayName:
        displayName ||
        `${cell(row, "first_name") || "Unknown"} ${cell(row, "last_name") || "Unknown"}`,
      firstName:
        cell(row, "first_name", "common_first_name") || fallback.firstName,
      lastName: cell(row, "last_name") || fallback.lastName,
      position:
        cell(roster, "position", "position_group") ||
        cell(row, "position", "position_group") ||
        null,
      ...
      yearsExperience:
        numberCell(roster, "years_exp", "years_experience") ??
        numberCell(row, "years_of_experience", "years_exp"),
...
export function planPlayerSync(
  incoming: NflversePlayer[],
  owned: ExistingOwnedPlayer[],
): PlayerSyncPlan {
  const issues: PlayerSyncIssue[] = [];
  const actions: PlayerSyncAction[] = [];
  const stable = new Map(
    owned
      .filter((player) => player.gsisId)
      .map((player) => [player.gsisId!, player]),
  );
  ...
  for (const player of incoming) {
    if (!player.gsisId) {
      issues.push({
        ...
        message: "nflverse row has no GSIS ID; it was not imported.",
        candidatePlayerIds: [],
      });
      continue;
    }
    ...
    const candidates =
      exact.get(
        matchKey(player.displayName, player.position, player.nflTeam),
      ) ?? [];
    if (candidates.length === 1) {
      ...
      issues.push({
        ...
        message:
          "Multiple owned FOFL players match name, position, and NFL team; manual review is required.",
  ...
  for (const player of owned.filter((item) => item.owned)) {
    if (!matchedOwned.has(player.playerId))
      issues.push({
        gsisId: player.gsisId,
        displayName: player.displayName,
        code: "owned_player_unmatched",
        message:
          "Existing FOFL-owned player was not safely matched to nflverse.",
  ...
  return {
    ...
    matchedAutomatically: actions.filter(
      (action) => action.kind === "link_existing",
    ).length,
    unmatchedCount: issues.filter(
      (issue) => issue.code === "owned_player_unmatched",
    ).length,
    reviewCount: issues.filter(
      (issue) => issue.code !== "owned_player_unmatched",
    ).length,
  };
}
