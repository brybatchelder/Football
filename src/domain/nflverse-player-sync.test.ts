...
describe("nflverse player sync", () => {
  it("parses quoted CSV values", () => {
    expect(
      parseCsv(
        'gsis_id,display_name,college_name\n00-1,"Smith, John","Miami, FL"\n',
      ),
    ).toEqual([
      {
        gsis_id: "00-1",
        display_name: "Smith, John",
        college_name: "Miami, FL",
      },
    ]);
  });
  ...
  it("overlays current roster attributes by GSIS ID", () => {
    const players = parseCsv(
      "gsis_id,display_name,position,rookie_year\n00-1,John Smith,WR,2026\n",
    );
    const rosters = parseCsv(
      "gsis_id,team,position,years_exp,status\n00-1,GB,WR,0,ACT\n",
    );
    expect(normalizeNflverseData(players, rosters)[0]).toMatchObject({
      gsisId: "00-1",
      nflTeam: "GB",
      yearsExperience: 0,
      isActive: true,
  ...
  it("links one exact owned player but sends ambiguous matches to review", () => {
    ...
    expect(plan.actions).toContainEqual(
      expect.objectContaining({ kind: "link_existing", playerId: "one" }),
    );
    expect(plan.issues).toContainEqual(
      expect.objectContaining({
        code: "ambiguous_match",
        candidatePlayerIds: ["two", "three"],
      }),
    );
  });
});
