...
export type PlayoffStatus =
  "division_leader" | "wild_card" | "first_out" | "in_the_hunt";
...
export function compareRaceTeams(
  left: PlayoffRaceTeam,
  right: PlayoffRaceTeam,
) {
...
export function seedPlayoffField(
  ...
  const wildCards = teams
    .filter((team) => !leaderIds.has(team.id))
    .sort(compareRaceTeams)
  ...
  const firstOut = teams
    .filter((team) => !fieldIds.has(team.id))
    .sort(compareRaceTeams)[0];
  return {
    field: field.map((team, index) => ({
      ...team,
      seed: index + 1,
      status: leaderIds.has(team.id)
        ? ("division_leader" as const)
        : ("wild_card" as const),
    })),
    firstOut: firstOut
      ? { ...firstOut, status: "first_out" as const }
      : undefined,
    inTheHunt: teams
      .filter((team) => !fieldIds.has(team.id) && team.id !== firstOut?.id)
      .sort(compareRaceTeams)
      .map((team) => ({ ...team, status: "in_the_hunt" as const })),
  };
}
...
export function projectedBracket(
  ...
  return {
    byes,
    matchups: Array.from(
      { length: Math.floor(playable.length / 2) },
      (_, index) => ({
...
export function simulatePlayoffOdds(
  ...
  const totals = new Map(
    initialTeams.map((team) => [
      team.id,
      { playoff: 0, division: 0, bye: 0, championship: 0 },
    ]),
  );
