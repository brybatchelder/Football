...
const ROSTERS_URL = (season: number) =>
...
export async function getPlayerSyncHistory() {
  return getDb()
    ...
    .from(playerSyncRuns)
    .orderBy(desc(playerSyncRuns.startedAt))
    .limit(8);
}
...
async function downloadCsv(url: string) {
  const response = await fetch(url, {
    cache: "no-store",
    headers: { "user-agent": "FOFL-player-sync/1.0" },
  });
  if (!response.ok)
    throw new Error(
      `nflverse returned ${response.status} for ${new URL(url).pathname}`,
    );
  const text = await response.text();
  if (!text.includes("gsis_id"))
    throw new Error(
      `nflverse source failed validation: ${new URL(url).pathname}`,
    );
  return text;
}
...
export async function syncNflversePlayers({
  dryRun,
  season,
}: {
  dryRun: boolean;
  ...
  const db = getDb();
  const [run] = await db
    .insert(playerSyncRuns)
    .values({ status: "running", dryRun, sourceSeason: season })
    .returning({ id: playerSyncRuns.id });
  try {
    const [playerCsv, rosterCsv] = await Promise.all([
      downloadCsv(PLAYERS_URL),
      downloadCsv(ROSTERS_URL(season)),
    ]);
    ...
    if (incoming.length < 1_000)
      throw new Error(
        `nflverse source failed validation: only ${incoming.length} player rows`,
      );
    ...
    const existingRows = await db
      ...
      .leftJoin(
        playerSeasons,
        and(
          eq(playerSeasons.playerId, players.id),
          eq(playerSeasons.year, season),
        ),
      )
      ...
      .leftJoin(
        rosterEntries,
        and(
          eq(rosterEntries.playerSeasonId, playerSeasons.id),
      ...
      await db.transaction(async (tx) => {
        ...
        for (const abbreviation of new Set(
          plan.actions.flatMap((action) =>
            action.player.nflTeam ? [action.player.nflTeam] : [],
          ),
        )) {
          const [team] = await tx
            .insert(nflTeams)
            .values({ abbreviation, city: abbreviation, name: abbreviation })
            .onConflictDoUpdate({
        ...
        for (const action of plan.actions) {
          ...
          if (action.kind === "create") {
            const [created] = await tx
              .insert(players)
              .values(playerValues(action.player))
              .returning({ id: players.id });
            playerId = created.id;
          } else if (playerId) {
            await tx
              .update(players)
              .set(playerValues(action.player))
          ...
          await tx
            ...
            .values({
              playerId,
              provider: "nflverse",
              externalId: action.player.gsisId,
            })
            .onConflictDoNothing();
          if (
            action.player.nflTeam ||
            action.player.position ||
            action.player.nflStatus
          ) {
            const [playerSeason] = await tx
              .insert(playerSeasons)
              .values({
                playerId,
                year: season,
                nflTeamId: action.player.nflTeam
                  ? teamIds.get(action.player.nflTeam)
                  : null,
            ...
            if (action.player.position) {
              await tx
                .delete(playerPositionEligibility)
                .where(
                  and(
                    eq(
                      playerPositionEligibility.playerSeasonId,
                      playerSeason.id,
                    ),
                    eq(playerPositionEligibility.isPrimary, true),
                  ),
              ...
              await tx
                .insert(playerPositionEligibility)
                .values({
    ...
    if (plan.issues.length)
      await db
        .insert(playerSyncIssues)
        .values(
          plan.issues.map((issue) => ({ playerSyncRunId: run.id, ...issue })),
        );
    const rosterAttributesUpdated = plan.actions.filter(
      (action) =>
        action.player.nflTeam ||
        action.player.position ||
        action.player.nflStatus,
    ...
    const result: PlayerSyncResult = {
      ...
      dryRun,
      sourceSeason: season,
      playersSeen: plan.playersSeen,
      playersCreated: plan.playersCreated,
      playersUpdated: plan.playersUpdated,
      rosterAttributesUpdated,
      matchedAutomatically: plan.matchedAutomatically,
      unmatchedCount: plan.unmatchedCount,
      reviewCount: plan.reviewCount,
      ownershipRecordsModified: 0,
    };
    await db
      .update(playerSyncRuns)
      .set({
        status: "succeeded",
